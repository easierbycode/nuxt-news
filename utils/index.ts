...
export const saveUserData = ({ idToken, expiresIn }, { email, avatar }) => {
    const tokenExpiration = String(Date.now() + expiresIn * 1000);
...
export const getUserFromCookie = req => {
    ...
    const expiresCookie = req.headers.cookie.split(';').find(c => c.trim().startsWith('expiresIn='));
    ...
    if (!jwtCookie || !expiresCookie || !userCookie || !avatarCookie)  return;

    const jwt = jwtCookie.split('=')[1];
    const expiresIn = expiresCookie.split('=')[1];
